...
public class BookingDAO {
    ...
    public boolean createPendingBooking(Booking booking, String paymentMethod, String paymentDetails) {
        ...
        try {
            ...
            try (PreparedStatement bookingPstmt = conn.prepareStatement(bookingQuery, new String[] { "booking_id" })) {
                ...
                bookingPstmt.setLong(3,
                        booking.getCheckIn().atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli());
                bookingPstmt.setLong(4,
                        booking.getCheckOut().atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli());
    ...
    public boolean modifyBooking(int bookingId, LocalDate newCheckOut, double newTotalPrice, double priceDifference) {
        ...
        try {
            ...
            String message = String.format("Booking #%d has been modified. New check-out date: %s", bookingId,
    ...
    public int cancelExpiredBookings() {
        ...
        int count = 0;
        // Use a new connection for this background task to avoid conflicts with the
        // main UI thread connection
